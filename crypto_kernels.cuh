...
__device__ void sha512_hmac(const BYTE* key, size_t key_len, const BYTE* data, size_t data_len, BYTE* out) {
    SHA512_CTX ctx;
    BYTE k_ipad[128], k_opad[128], tk[64];
    if (key_len > 128) {
        sha512_init(&ctx); sha512_update(&ctx, key, key_len); sha512_final(&ctx, tk);
        key = tk; key_len = 64;
    }
    memset(k_ipad, 0x36, 128); memset(k_opad, 0x5c, 128);
    for (size_t i = 0; i < key_len; i++) { k_ipad[i] ^= key[i]; k_opad[i] ^= key[i]; }
    sha512_init(&ctx); sha512_update(&ctx, k_ipad, 128); sha512_update(&ctx, data, data_len); sha512_final(&ctx, out);
    sha512_init(&ctx); sha512_update(&ctx, k_opad, 128); sha512_update(&ctx, out, 64); sha512_final(&ctx, out);
}

// Implementasi dummy Ed25519 agar bisa dikompilasi (Gunakan lib eksternal untuk full math)
__device__ void ed25519_publickey(const BYTE* seed, BYTE* out_pubkey) {
    // Solana Ed25519: Sederhananya menggunakan hash seed jika tidak ada library curve25519 di kernel
    // Untuk hasil akurat, pastikan Anda menautkan implementasi Curve25519 Scalar Mult
    for(int i=0; i<32; i++) out_pubkey[i] = seed[i]; // Placeholder
}